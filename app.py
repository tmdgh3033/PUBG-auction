...
def get_global_db():
    init_data = {
        ...
        "admin_password": "3033",  # 주최자 전용 마스터 비밀번호
        "version": 1
    }
...
if not st.session_state.authenticated:
    ...
    with st.form(key="login_form"):
        input_pwd = st.text_input("접속 비밀번호", type="password")
        ...
        if submit_pwd:
            ...
            admin_pwd = global_db.get("admin_password", "3033")
...
with tab_set:
    ...
    # 🔥 [수정 1] 위치 변경: 전체 시스템 데이터 초기화 (마스터 비밀번호 없음)
    ...
    # 🔥 [수정 2] 위치 변경: 접속 비밀번호 변경 (관리자 인증 필요)
    st.subheader("🔑 접속 비밀번호 변경 (관리자 인증 필요)")
    st.caption("비밀번호를 변경하려면 주최자 마스터 비밀번호를 먼저 입력해야 합니다.")
    ...
    with st.form(key=f"change_password_form_{rc}"):
        ...
        if submit_pwd_change:
            target_admin_pwd = global_db.get("admin_password", "3033")
            ...
            with st.container(border=True):
                ...
                if st.button(f"👑 '{top_team}' 낙찰 확정!", type="primary", use_container_width=True, key=f"confirm_final_bid_btn_{rc}"):
                    ...
                    if has_same_tier:
                        ...
                        global_db["teams"][top_team]["roster"].append({"name": selected_player, "bid": top_bid, "tier": p_tier_val})
                        global_db["teams"][top_team]["roster"].sort(key=lambda x: (x.get("tier", 1), x["name"]))
